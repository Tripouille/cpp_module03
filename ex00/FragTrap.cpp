...
void			FragTrap::stunningAttack(std::string const &target) const
{
	std::cout << "FR4G-TP " << _name <<  " attacks " << target << " with a stunning hit " << std::endl;
}

void			FragTrap::viciousAttack(std::string const &target) const
{
	std::cout << "FR4G-TP " << _name <<  " attacks " << target << " in the back causing "
				<< _meleeAttackDamage * 2 << " points of damage!" << std::endl;
}

void			FragTrap::rocketAttack(std::string const &target) const
{
